...
README、`docs/adr/` 配下のADR、`migrations/README.md` などソースコード
以外のドキュメントは、日本語で記述する。ソースコード自体（Go・TypeScript
のコード、コメント、識別子）およびコミットメッセージは対象外とし、既存の
言語慣習に従う。

Vulnerability、Finding、Risk、Remediation等のドメイン用語や、テーブル名・
関数名・パッケージ名などのコード上の識別子は、無理に訳さず原語（英語）の
まま用いてよい。
